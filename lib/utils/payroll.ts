...
export function calculateMonthlyWage(
  ...
  // 日別明細を作成
  const dailyBreakdown: DailyWageBreakdown[] = confirmedAttendances.map((attendance) => {
  ...
  // 案件別集計
  const projectWageMap = new Map<string, ProjectWageBreakdown>();
  ...
  const projectBreakdown = Array.from(projectWageMap.values());
